...
export function buildEndpoints(request: Request) {
	return {
		raw: request,
		students: {
		...
		me: {
			...
			eduPlaces: {
				...
				remove: (id: number) =>
					request<void>(`/me/edu-places/${id}`, { method: 'DELETE' })
			},
			workPlaces: {
				...
				remove: (id: number) =>
					request<void>(`/me/work-places/${id}`, { method: 'DELETE' })
			},
			socials: {
			...
			keySkills: {
				...
				add: (skillId: number) =>
					request<Skill>(`/me/key-skills/${skillId}`, { method: 'POST' }),
			...
			softSkills: {
				...
				add: (skillId: number) =>
					request<Skill>(`/me/soft-skills/${skillId}`, { method: 'POST' }),
				remove: (skillId: number) =>
